...
MaterialDelegate {
    ...
    title: qsTr("Diffuse & Specular Map")

    Connections {
        target: materialRepeater.model
        onRoleDataChanged: {
            if (roleIndex === EditorSceneItemMaterialComponentsModel.MaterialDiffuseTextureUrl)
                diffuseTextureInputField.currentValue = materialDiffuseTextureUrl
            else if (roleIndex === EditorSceneItemMaterialComponentsModel.MaterialSpecularTextureUrl)
                specularTextureInputField.currentValue = materialSpecularTextureUrl
        }
    }
    ...
    TextureInputField {
        id: specularTextureInputField
        label: qsTr("Specular Map")
        componentType: thisDelegate.componentType
